...
flowchart TB
    ...
    subgraph CoAgent["CoAgent"]
        subgraph UI["表现层"]
            ADMIN["管理台<br/>总览 · 工作台 · 审计"]
            API["API 服务"]
        end

        ORCH["编排器<br/>幂等 · 超时 · 降级"]

        subgraph Pipeline["处置流水线"]
            direction LR
            R["场景路由"]
            PB["处置手册+工具"]
            AI["根因推理"]
...
| 层级 | 职责 |
|---|---|
| **表现层** | 管理台三 Tab：事故总览、处置工作台、审计复盘 |
| **编排层** | 事件幂等、链路超时、全链路审计与分层降级 |
| **处置层** | 场景路由 → 手册约束下的工具查询与根因推理 → 把握度评分 |
| **基础设施** | 实时 SSE 时间线、SQLite 持久化、配置驱动 |
| **数据层** | 处置手册与场景配置（JSON 单一真相源） |

交互式架构图（含业务流程视图、流水线自动高亮）：[docs/diagrams/coagent-architecture.html](docs/diagrams/coagent-architecture.html)
