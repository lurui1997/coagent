...
idea.md 要求：**27h 只打磨一条路径，但 Demo 可用多场景展示「因果一跳」的不同形态。** CoAgent 用 S1/S2/S3 展示同一创新内核的三种 Agent Ops 切面，**不是三条独立产品**。
...
> **范围警告：** 不为「丰富」而演示 infra 级 K8s 级联故障（idea.md 候选 1 完整版）——那是另一条 27h 做不完的线。CoAgent 的「因果」落在 **Agent 运行态层**，与当前 Spec 工程范围一致。
